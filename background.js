const GITHUB_REPO = "hugo-rossel/amp-r";
...
async function checkForUpdates(force = false) {
    try {
        ...
        if (!force && storage.lastCheckTime && (now - storage.lastCheckTime < CHECK_INTERVAL)) {
            console.log("ÅMP-r : Vérification récente, reportée.");
            return;
        }

        console.log("ÅMP-r : Vérification des mises à jour sur GitHub...");
        ...
        console.log(`ÅMP-r : Dernière version GitHub trouvée : ${cleanVersion}`);
    } catch (error) {
        console.error("ÅMP-r : Échec de la vérification de mise à jour", error);
    }
}
